...
const metrics = [
  { 
    ...
    sublabel: "Custom Enterprise Solutions",
  },
  { 
    value: 450, 
    suffix: "+", 
    ...
    sublabel: "Workflow & Operations",
  },
  { 
    value: 45, 
    suffix: "+", 
    prefix: "",
    label: "Voice Agents Deployed",
    sublabel: "Inbound & Outbound Calling",
  },
  { 
    value: 18, 
    suffix: "+", 
  ...
  { 
    ...
    prefix: "",
    label: "System Uptime",
    sublabel: "Reliability Guarantee",
  },
];
...
function AnimatedNumber({ end, suffix = "", prefix = "" }: { end: number; suffix?: string; prefix?: string }) {
  ...
  return (
    <div ref={ref} className="inline-flex items-baseline">
      ...
      <span className="text-[#eca8d6] font-display">{suffix}</span>
    </div>
  );
}
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-12 gap-8 mb-20 lg:mb-24">
          <div className="lg:col-span-8 lg:col-start-1">
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[100px] font-display tracking-tight leading-[0.95] transition-all duration-1000 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {metrics.map((metric, index) => (
            <div
              key={metric.label}
              className={`bg-foreground/[0.02] border border-foreground/10 p-6 flex flex-col justify-between gap-6 transition-all duration-700 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
              style={{ transitionDelay: `${index * 80}ms` }}
            >
              <div>
                <div className="text-[10px] text-muted-foreground font-mono mb-2 uppercase tracking-wider">{metric.sublabel}</div>
                <div className="text-sm text-foreground/80 mb-3 font-semibold h-10 flex items-center">{metric.label}</div>
                <DotGraph
                  color={index % 2 === 0 ? "green" : "white"}
                  height={24}
                  freq1={0.25 + index * 0.05}
                  freq2={0.08 + index * 0.02}
                  freqT={0.6 + index * 0.1}
              ...
              <div className="text-4xl lg:text-5xl font-display tracking-tight w-full mt-4">
        ...
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          ...
          <span className="text-foreground">+8 more enterprise integrations</span>
        </div>
      </div>
    </section>
  );
